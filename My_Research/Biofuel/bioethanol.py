...
biofuel_production_db = bw.Database("Biofuel Database")
print(bw.databases)
print('==========')

# 删除多余数据库
# del bw.databases['biofuel database']

# 写入生产流程
production_process = {
    ("Biofuel Database", "code for bioethanol Production"): {
        ...
        'exchanges': [{
            'amount': 8.75,
            'input': ('ecoinvent_3.6_apos', '10e0d83a4122ddb63bddca4e7804da30'),  # cellulase enzyme
            'type': 'technosphere',
            'uncertainty type': 2,
            'unit': 'kg'}, {
            'amount': 270.67,
...
# 向数据库写入数据
biofuel_production_db.write(production_process)

print('==========')

# 打印工序并写入列表
renewable_energy_data_list = []

biofuel_prod = [act for act in biofuel_production_db if 'bioethanol' in act['name']][0]
print(biofuel_prod)
renewable_energy_data_list.append(biofuel_prod)
